...
async fn install_plugin(discovered: DiscoveredPlugin) -> puniyu_error::Result {
    ...
    init_commands(index, discovered.instance.commands()).unwrap_or_else(|e| {
        panic!("Failed to register command for plugin {}: {:?}", name, e)
    });

    init_tasks(index, discovered.instance.tasks()).await.unwrap_or_else(|e| {
        panic!("Failed to register task for plugin {}: {:?}", name, e)
    });
    if let Some(server) = discovered.instance.server() {
        super::server::init_server(source, server)
