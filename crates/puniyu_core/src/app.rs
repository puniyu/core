...
impl AppBuilder {
    ...
    /// 添加自定义配置
    pub fn with_config<C: puniyu_config::Config + Default + 'static>(
        mut self,
        config: C,
    ) -> Self {
...
impl App {
    ...
    pub async fn run(self) -> io::Result<()> {
        ...
        let resolved = resolve::resolve(all_sets).map_err(|e| {
            std::io::Error::other(format!("Failed to resolve components: {}", e))
        })?;
